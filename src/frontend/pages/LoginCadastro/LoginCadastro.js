...
function LoginCadastro() {
  ...
  const handleSubmit = async (e, tipoFormulario) => {
    e.preventDefault();
    const erro = tipoFormulario === 'cadastro' ? validarCadastro() : validarLogin();
    setMensagemErro(erro);
    if (erro) return;

    if (tipoFormulario === 'cadastro') {
      alert('Dados cadastrados com sucesso!');
      setUsername(''); setTelephone(''); setEmail(''); setSenha(''); setConfirmaSenha('');
    } else {
      try {
        ...
        if (response.ok && data.success) {
          if (lembreDeMim) localStorage.setItem('emailSalvo', email);
          localStorage.setItem('userID', data.userID);
          alert('Login concluído!');
          navigate('/dashboard');
        } else alert(data.message || 'Erro ao autenticar!');
      } catch {
        alert('Erro na comunicação com o servidor.');
      }
      setEmail(''); setSenha(''); setLembreDeMim(false);
    }
  };
